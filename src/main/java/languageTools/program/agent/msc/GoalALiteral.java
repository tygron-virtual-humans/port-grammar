...
public class GoalALiteral extends MentalLiteral {
	...
	public GoalALiteral(boolean polarity, Selector selector, Query query, SourceInfo info) {
		super(polarity, selector, query, info);
	}

	@Override
	public MentalLiteral applySubst(Substitution substitution) {
		return new GoalALiteral(polarity, selector.applySubst(substitution), query.applySubst(substitution), info);
	}
