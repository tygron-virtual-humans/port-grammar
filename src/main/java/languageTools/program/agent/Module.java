 ...
 * </p>
 * <p>
 * Use {@link AgentValidator} to create a module from a text file
 * with extension 'mod2g'.
 * </p>
 * 
...
public class Module extends Program {
	 ...
	 * 
	 * <p>For anonymous modules, the parameters are derived by the validator
	 * from the variables present in the rule that calls/triggers it.</p>
	 */
	...
	/**
	 * <p>List of imported files.</p>
	 * 
	 * <p>It should be possible to resolve the location of files that are imported
	...
	/**
	 * @param name A name for this {@link Module}.
	 */
	...
	/**
	 * @param parameter The parameter that is added to the module.
	 */
	...
	/**
	 * @param focusMethod The focus method that is used when entering this module.
	 */
	...
	/**
	 * @param exitCondition The exit condition used to check when to exit this module.
	 */
	...
	/**
	 * @param file A file that is imported by this {@link Module}.
	 */
	...
	/**
	 * @param knowledge The knowledge used within this {@link Module}.
	 */
	...
	/**
	 * @param beliefs The beliefs used within this {@link Module}.
	 */
	...
	/**
	 * @param goals The goals used within this {@link Module}.
	 */
	...
	/**
	 * @param macros The macros specified at the beginning of the program section. 
	 */
	...
	/**
	 * @param rules The rules in the program section of this module.
	 */
	...
	/**
	 * @return The action specifications locally defined within this {@link Module}.
	 */
	...
	/**
	 * @param spec The action specifications in the action specification section of this module.
	 */
	...
	/**
	 * @param type A type for this {@link Module}.
	 */
	...
	public Module applySubst(Substitution substitution) {
		...
		// Of course, only applying ground terms simplifies a lot... A module should be closed when executed!

		return new Module(name, type, kri, getSourceInfo());
	}
	...
	public String toString(String linePrefix, String indent) {
		...
		str.append(linePrefix + "<module: " + this + "[focus=" + focusMethod + ", exit=" + exitCondition + "]\n");
		str.append(linePrefix + indent + "<type: " + type + ">,\n");
		...
		Iterator<DatabaseFormula> knows = knowledge.iterator();
		while (knows.hasNext()) {
		...
		Iterator<DatabaseFormula> believes = beliefs.iterator();
		...
		Iterator<Query> wants = goals.iterator();
		while (wants.hasNext()) {
		...
		str.append(linePrefix + indent + "<rule evaluation order: " + order + ">,\n");
		
		// Macros
		str.append(linePrefix + indent + "<macros:\n");
		Iterator<Macro> macro = macros.iterator();
		while (macro.hasNext()) {
			str.append(macro.next().toString(linePrefix + indent + indent, indent));
		...
		Iterator<Rule> rule = rules.iterator();
		while (rule.hasNext()) {
		...
		Iterator<ActionSpecification> spec = specs.iterator();
		while (spec.hasNext()) {
			str.append(spec.next().toString(linePrefix + indent + indent, indent));
	 ...
	 * 	<li>{@link #ANONYMOUS}: An anonymous module, derived from <code>{ ... }</code> blocks.</li>
	 * 	<li>{@link #USERDEF}: A user-defined module.</li>
	 * </ul>
	 * </p>
	 */
	public enum TYPE {
		PROGRAM("program"), INIT("init"), MAIN("main"), EVENT("event"), ANONYMOUS("anonymous"), USERDEF("user-defined");
	 ...
	 * 
	 * <p>This condition is evaluated each time that the rules of the module
	 * have been evaluated (according to the rule evaluation order associated
	 * with the module). That is, <i>after</i> the rules have been evaluated,
	 * it is checked whether the module should be exited.</p>
	 * 
	 * <p>Note that modules can also be exited using the <code>exit-module</code> action.</p>
	 */
