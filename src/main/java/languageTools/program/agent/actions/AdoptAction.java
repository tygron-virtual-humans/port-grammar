...
public class AdoptAction extends MentalAction {
	 ...
	 *
	 * <p>
	 * An adopt action can be performed if the agent does not believe that the
	 * goal that is adopted has already been achieved, i.e., believes it already
	 * to be the case, and the goal does not follow from one of the goals that
	 * are already present in the goal base.
	 * </p>
	 *
	...
	public MentalStateCondition getPrecondition() {
		...
		formulalist.add(new BelLiteral(false, getSelector(), query,
				getSourceInfo()));
		// Construct the goal part of the query: NOT(GOAL(query)).
		formulalist.add(new GoalLiteral(false, getSelector(), query,
				getSourceInfo()));
		// Combine both parts.
