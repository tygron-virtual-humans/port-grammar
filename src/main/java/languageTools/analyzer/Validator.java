...
/**
 * A validator parses and validates a program file, i.e., checks for both syntactic and semantic issues.
 * During the validation process a {@link Program} is created that can be obtained by {@link #getProgram()}
 * after calling {@link #validate()}.
 * 
 * {@link #validate()} may generate {@link ParserError}s if the parser detected problems and {@link ValidatorError}s
 * if the program specific validator detected problems. It may also generate {@link ValidatorWarning}s. Upon
 * completion, {@link #validate()} sets a flag in the program which indicates whether the program is valid or not.
 * Note that this does not mean that the program <i>can be run</i>, see also {@link Program#canRun()}.
 */
public abstract class Validator<L extends MyLexer<?>, P extends Parser,
		E extends MyErrorStrategy, Q extends Program> implements ANTLRErrorListener {
	...
	protected abstract L getNewLexer(CharStream stream, ANTLRErrorListener errorlistener);
	...
	public ParseTree parseFile() throws IOException {
		...
		// Create a lexer that feeds off of input CharStream (also redirects error listener).
	...
	public void validate() {
		try {
			...
			// Set validity flag; a program is valid if it did not generate any parsing or validation errors
			this.program.setValid(getSyntaxErrors().isEmpty() && getErrors().isEmpty());
		} catch (Exception e) {
			// Convert stack trace to string
			StringWriter sw = new StringWriter();
			e.printStackTrace(new PrintWriter(sw));
			reportError(SyntaxError.FATAL, null, e.getMessage() + "\n" + sw.toString());
		}
	}
	 ...
	 * 
	 * @param tree Parse tree of program.
	 */
	...
	/**
	 * Reports the results of the validation, listing all syntax and validation errors and warnings.
	 */
	public String report() {
		// Sort errors on line and position numbers
		syntaxErrors = sort(syntaxErrors);
		errors = sort(errors);
	 ...
	 * 
	 * @param error The type of syntax error that is added.
	 * @param info Source info object.
	 * @param args Additional info to be inserted into warning message.
	 */
	protected boolean reportError(SyntaxError type, SourceInfo info, String... args) {
		return syntaxErrors.add(new ParserError(type, info, args));
	}
	 ...
	 * 
	 * @param error The type of syntax error that is added.
	 * @param pos The input stream position where the error was detected.
	 * @param args Additional info to be inserted into warning message.
	 */
	protected boolean reportError(SyntaxError type, InputStreamPosition pos, String... args) {
		return syntaxErrors.add(new ParserError(type, pos, args));
	}
	...
	public boolean reportError(ValidatorErrorType type, ParserRuleContext context, String... args) {
		return errors.add(new ValidatorError(type, getSourceInfo(context), args));
	}
	...
	protected boolean reportError(ValidatorErrorType type, TerminalNode node, String... args) {
		return errors.add(new ValidatorError(type, getSourceInfo(node), args));
	}
	
	/**
	 * @param ctx An ANTLR {@link ParserRuleContext}.
	 * @return A source info object ({@link InputStreamPosition}) with information extracted from the rule context.
	 */
	public SourceInfo getSourceInfo(ParserRuleContext ctx) {
		InputStreamPosition pos =
				new InputStreamPosition(ctx.getStart(),
	 ...
	 * @return A source info object ({@link InputStreamPosition}) with information extracted from the terminal node.
	 */
	public SourceInfo getSourceInfo(TerminalNode node) {
		return new InputStreamPosition(node.getSymbol(), node.getSymbol(), source);
	}
	 ...
	 * ANTLR ParserRuleContext object.
	 * 
	 * @param warning The warning that is added.
	 * @param info A source info object.
	 * @param args Additional info to be inserted into warning message.
	 */
	public boolean reportWarning(ValidatorWarningType type, SourceInfo info, String... args) {
		return warnings.add(new ValidatorWarning(type, info, args));
	}
	...
	public boolean reportWarning(ValidatorWarningType type, ParserRuleContext context, String... args) {
		return warnings.add(new ValidatorWarning(type, getSourceInfo(context), args));
	}
	...
	protected boolean reportWarning(ValidatorWarningType type, TerminalNode node, String... args) {
		return warnings.add(new ValidatorWarning(type, getSourceInfo(node), args));
	}
	...
	public void syntaxError(Recognizer<?, ?> recognizer,
		...
		InputStreamPosition pos = new InputStreamPosition(line,
				charPositionInLine, start, stop, source);
	 ...
	 * @param stop stopIndex of last recognition error
	 * @param text character(s) that could not be recognized
	 */
	public void handleLexerError(Recognizer<?, ?> recognizer, Object offendingSymbol, 
			InputStreamPosition pos, String text, RecognitionException e) {
		...
		} else { // if nothing else, by default, assume token recognition problem
			type = SyntaxError.TOKENRECOGNITIONERROR;
		}
		
		switch(type) {
		case TOKENRECOGNITIONERROR:
			// Check if this and last error were both token recognition errors; if so, merge them
			int last = syntaxErrors.size()-1;
			if (last >= 0 && syntaxErrors.get(last).getType().equals(type)) {
				Message error = syntaxErrors.get(last);
		...
		case UNTERMINATEDSINGLEQUOTEDSTRINGLITERAL:
			...
			break;
		default:
			throw new UnsupportedOperationException("Unexpected lexer error: " + text, e);
		}

		// Remove tabs and newlines
		text = removeTabsAndNewLines(text);

		syntaxErrors.add(new ParserError(type, pos, text));
	}
	
	/**
	 * Adds error for parsing problem.
	 * 
	 * <p>Simply pushes parser error msg forward. See {@link #MASErrorStrateg} for handling of parsing errors.</p> 
	 * 
	 * @param pos input stream position
	 * @param msg reported parser error msg
	 */
	public void handleParserError(Recognizer<?, ?> recognizer,
			Object offendingSymbol, InputStreamPosition pos, String expectedtokens,
	...
	public void reportAmbiguity(Parser recognizer, DFA dfa, int startIndex,
		...
		System.out.println("Found ambiguous readings of file! Please report this finding and send us this file.");
	}
	...
	/**
	 * Appends path of source file to filename; files should be located relative to source file.
	 * 
	...
	private List<Message> sort(List<Message> messages) {
		...
		while (msgIterator.hasNext()) {
			...
			if (msg.getSource() != null) {
				...
				while (i < sorted.size() && before(sorted.get(i).getSource(), msg.getSource())) {
						i++;
				}
			}
			sorted.add(i, msg);
	 ...
	 * @return {@code true} if source position of info1 object occurs before position of info2 object.
	 */
	private boolean before(SourceInfo info1, SourceInfo info2) {
		boolean source = (info1.getSource().getName().compareTo(info2.getSource().getName()) < 0);
		boolean sourceEqual = (info1.getSource().getName().compareTo(info2.getSource().getName()) == 0);
		...
		boolean position = sourceEqual && lineNrEqual 
	 ...
	 * @return string with comma separated list of set items, or plain single item, or empty string
	 */
